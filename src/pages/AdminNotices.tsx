...
import type { AdminNoticeSummary } from '../notice/noticeTypes'
...
export function AdminNotices() {
  ...
  const [rows, setRows] = useState<AdminNoticeSummary[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const j = await readJsonResponse<{ notices?: AdminNoticeSummary[]; error?: string }>(r)
      if (r.status === 401) {
        clearAdminToken()
  ...
  const onDelete = async (id: number) => {
    ...
    setError(null)
    try {
      const r = await apiFetch(`/api/admin/notices/${encodeURIComponent(String(id))}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${t}` },
      })
      ...
      <main className="adminMain">
        <div className="container adminInner">
          <div className="adminHead">
            ...
            <p className="adminHint muted">공지 목록을 확인하고, 수정/삭제할 수 있습니다.</p>
          </div>
                    ...
                    {pager.total === 0 && !loading ? (
                      ...
                      pager.pageItems.map((row) => (
                        <tr key={row.id}>
                          ...
                          <td>
                            <Link to={`/admin/notices/${row.id}`} className="linkBtn">
                              수정
                            </Link>
                          </td>
                          <td>
